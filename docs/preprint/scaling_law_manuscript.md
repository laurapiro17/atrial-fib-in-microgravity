...
**Methods.** A continuous, literature-anchored map sends `g` to atrial remodelling through the cephalad fluid-shift drive `(1 − g)`, calibrated so `g=1` recovers a validated ground baseline and `g=0` recovers an established microgravity operating point. Single-cell APD₉₀ was measured with the Courtemanche–Ramirez–Nattel (CRN) human-atrial ionic model across the remodelling continuum (one vectorised pacing run), forming `𝒩_g(g)`. The prediction was corroborated in a 2-D monodomain reaction–diffusion sheet (run at reduced inter-cellular coupling so rotor cores resolve on a tractable grid, hence at a conduction velocity below the 58 cm/s single-cell calibration; the same `g → remodelling` map drives both models) by measuring rotor (phase-singularity) burden at each gravity level. Sensitivity to the two main assumptions (atrial path length `L₀`; fluid-shift map shape) was quantified.

**Results.** `𝒩_g` rose monotonically as gravity fell, from 0.47 at Earth to 1.29 in free fall, crossing unity at **`g* ≈ 0.22`** — between the Moon (`𝒩_g ≈ 1.07`, above threshold) and Mars (`𝒩_g ≈ 0.82`, below threshold *in the analytic model*). Importantly, the full-sheet model does **not** confirm that Mars is protected: rotor burden increased as gravity decreased and was exactly zero at Earth, but at the Mars level it remained as high as at the Moon (overlapping CIs), with only a gradual transition. The analytic threshold is thus an optimistic bound, not a confirmed safe line. `g*` was robust to the map shape (`g* ∈ [0.13, 0.34]` for drive exponents 0.5–2.0) but strongly dependent on atrial size: no vulnerability at `L₀ = 6 cm`, `g* = 0.22` at 8 cm, 0.41 at 10 cm, and 0.58 at 12 cm (Mars vulnerable).
...
**Full-sheet corroboration.** A 2-D monodomain reaction–diffusion sheet (CRN ionics; spatially correlated fibrosis) was built at each gravity level with severity, fibrosis density and dilation all driven by the same `g → remodelling` map — so this is a consistency check of that shared map under spatially resolved dynamics, not an independent test of it. To keep rotor cores resolvable on a tractable grid the sheet uses a reduced longitudinal coupling (`d_long = 0.06`), giving a conduction velocity below the 58 cm/s single-cell calibration; the corroboration is therefore a qualitative direction-of-effect check, not matched in CV to the analytic model. A broken wavefront was induced and rotor burden quantified as phase-singularity density via the topological-charge method on a delayed-voltage phase field.
...
**The scaling law.** `𝒩_g` rose monotonically as gravity fell (Table 1, `figures/gravity_law.png`), crossing unity at **`g* ≈ 0.22`**. The Moon (0.16 g) lies just inside the vulnerable region (`𝒩_g ≈ 1.07`); Mars (0.38 g) lies just outside it in the analytic model (`𝒩_g ≈ 0.82`) — but, as the corroboration below shows, the full sheet does **not** confirm Mars is protected; interplanetary transit (0 g) is the most vulnerable (`𝒩_g = 1.29`); Earth is safe (`𝒩_g = 0.47`).
...
A one-line dimensionless argument predicts a critical gravity for atrial re-entrant vulnerability that falls **between the Moon and Mars**, and a spatially resolved reaction–diffusion model reproduces the trend (Earth clean, vulnerability rising as gravity falls), though it does not confirm the precise threshold. The prediction is consistent with the clinical picture — increased vulnerability without sustained AF (Khine et al., 2018) — because `𝒩_g` indexes *substrate proximity to re-entry*, not guaranteed fibrillation.
...
- **The microgravity electrical remodelling is parameterised as full chronic-AF-type ionic remodelling** (I_CaL −70%, I_to/I_Kur −50%, I_K1 +100%), shortening APD₉₀ from ≈294 to ≈139 ms (>50%) at `g = 0`. There is **no spaceflight evidence the atrial APD shortens anywhere near this much** — Khine et al. report only modest, transient change and no sustained AF. This single choice sets the magnitude of the entire `𝒩_g(g)` curve and is very likely an overestimate; the absolute position of `g*` should be read with that caveat.
- **The `g → stretch / remodelling` map is the weakest link**, and it is shared by both models, so the sheet "corroboration" tests the *consequences* of the map under spatially resolved dynamics — not the map itself. The linear form and the acute-stretch mechano-electric sensitivities (used as a stand-in for chronic adaptation) are hypotheses, not data. This is the first thing a partial-gravity experiment should test.
- **Single ionic model, 2-D** for corroboration (6-seed ensemble, but no autonomic dynamics and no 3-D atrial geometry), run at reduced coupling/CV to resolve rotors. The sheet result is a direction-of-effect check, not a calibrated incidence estimate, and its confidence intervals do not separate transit, Moon and Mars.
