...
**Results:** 𝒩_g rose monotonically as gravity fell (0.47 at Earth to 1.29 in free fall), crossing unity at g* ≈ 0.22 — between the Moon (𝒩_g ≈ 1.07, above threshold) and Mars (𝒩_g ≈ 0.82, below threshold in the analytic model only). The full sheet reproduced the trend (rotor burden zero at Earth, rising as gravity fell) but did **not** confirm Mars protection: Mars-level burden overlapped the Moon's. g* was robust to the assumed map shape (0.13–0.34) but strongly dependent on atrial size (no vulnerability at L₀ = 6 cm; Mars vulnerable at L₀ = 12 cm).
...
**Full-sheet corroboration.** A two-dimensional monodomain reaction–diffusion sheet (CRN ionics; spatially correlated fibrosis) was built at each gravity level with all tissue knobs driven by the same map; because both models share that map, this is a consistency check under spatially resolved dynamics rather than an independent test. To keep rotor cores resolvable on a tractable grid the sheet uses reduced longitudinal coupling (d_long = 0.06), giving a conduction velocity below the 58 cm/s single-cell calibration, so the corroboration is a qualitative direction-of-effect check not matched in CV to the analytic model. A broken wavefront was induced and rotor burden quantified as phase-singularity density via the topological-charge method on a time-delayed-voltage phase field.
...
𝒩_g rose monotonically as gravity fell (Table 1), crossing unity at g* ≈ 0.22. The Moon (0.16 g) lies just inside the vulnerable region (𝒩_g ≈ 1.07); Mars (0.38 g) lies just outside it in the analytic model (𝒩_g ≈ 0.82) — though the full sheet does not confirm Mars protection (see below); interplanetary transit (0 g) is most vulnerable (𝒩_g = 1.29); Earth is safe (𝒩_g = 0.47). Notably, although the model was not calibrated to wavelength data, its Earth wavelength (17.1 cm) falls within the normal range measured by Rensma et al. (14–18 cm) and its free-fall wavelength (8.1 cm) sits at their AF-induction threshold (≈7.8 cm) [4] — an unprompted, cross-species consistency check.
...
This study has clear limitations. First and most important, the microgravity electrical remodelling is parameterised as full chronic-AF-type ionic remodelling (I_CaL −70%, I_to/I_Kur −50%, I_K1 +100%), shortening APD₉₀ from ≈294 to ≈139 ms at g = 0; there is no spaceflight evidence the atrial APD shortens this much (Khine et al. report only modest, transient change and no sustained AF), so this single assumption sets the magnitude of the entire curve and is likely an overestimate. Second, the gravity → remodelling map is shared by both models, so the sheet is a consistency check of the map's consequences, not an independent test of the map; the map's linear form and its acute-stretch mechano-electric sensitivities (used for chronic adaptation) are hypotheses, not data. The corroboration uses a single ionic model in two dimensions at reduced coupling, without autonomic dynamics or three-dimensional geometry, and its confidence intervals do not separate transit, Moon and Mars. 𝒩_g is proposed, not proven; the analytic threshold is an optimistic bound that the full model does not confirm. No human partial-gravity electrophysiology exists to fix the absolute value of g*, so the headline g* ≈ 0.22 is conditional on L₀ ≈ 8 cm and the stated calibration. The framework is nonetheless falsifiable: partial-gravity measurements of atrial APD₉₀, CV and effective path length that yield an 𝒩_g not crossing unity within [0, 1] g, or crossing it far from the predicted band, would refute the hypothesis.
